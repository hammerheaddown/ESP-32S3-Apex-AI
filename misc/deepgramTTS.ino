...
void deepgramTTS(const char* textToSpeech) {
    ...
    if (SD.exists("/wakewords/temp.mp3")) {
        SD.remove("/wakewords/temp.mp3");
    }
    File mp3File = SD.open("/wakewords/temp.mp3", FILE_WRITE);
    if (!mp3File) {
        Serial.println("Failed to open /wakewords/temp.mp3 for writing");
        return;
    }
