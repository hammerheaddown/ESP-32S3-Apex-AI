...
#define VOLUME_SCALE 0.1  // Volume scaling factor (0.0 — silence, 1.0 — original level)
...
bool recordAudio(const char *path, uint32_t duration) {
    ...
    while (samples > 0 && isRecording) {
        size_t bytesRead = 0;
        i2s_channel_read(rx_handle, buffer32, BUFFER_SIZE * sizeof(int32_t), &bytesRead, portMAX_DELAY);
...
void finishedRecording(String filename) {
  ...
  if (fileSize <= 44) {  // 44 is the size of a WAV header without data
    ...
    Serial.println("File contains audio data, but Deepgram upload is disabled.");
